...
import polars as pl
from dags.exchange.exchange_detail.jobs.utils import ExchangeDetailPath, ExchangeHolidayPath
...
class ExchangeDetailsJobs:
    ...
    def extract_exchange_codes(file_path: str) -> list[str]:
        """
        Extracts exchange codes from processed exchange.
        """
    ...
    def download_details(exchange_code: str):
        """
        Retrieves details from api for a given exchange code and dumps it to data_lake raw.
        """
    ...
    def transform_holidays(file_path: str):
        ...
        Save each exchange uid to data_lake processed zone.


        """
        details_raw_data = json.loads(dl_client.download_file_into_memory(file_path))
        ...
        # some exchange, especially virtual ones like FOREX don't have holidays
