...
from services.hooks.postgres.types import FilterObject, FilterParams
...
class Filter:
    ...
    def __build_one_filter(self, filter_object: FilterObject) -> Composed:  # noqa
        ...
        if operator == "EQUAL":
            return self.__build_standard_filter_clause(
                column=column, value=value, operator=SQL("=")
            )
        if operator == "HIGHER":
            return self.__build_standard_filter_clause(
                column=column, value=value, operator=SQL(">")
        ...
        if operator == "IN":
            ...
            return SQL("{column} = ({value})").format(
                column=Identifier(column), value=value_list
            )
        return Composed("")
