...
DB_SCHEMA_MAPPING="mapping"
DB_SCHEMA_MIGRATION="migration"
DB_SCHEMA_DATA="data"
DB_NAME=$DB_NAME_STOCKS
...
psql -v ON_ERROR_STOP=1 --username "postgres" --dbname "postgres" <<-EOSQL
    ...
    BEGIN;
        CREATE SCHEMA $DB_SCHEMA_MAPPING AUTHORIZATION $DB_ADMIN_USER;

        REVOKE ALL ON SCHEMA $DB_SCHEMA_MAPPING FROM PUBLIC;
        GRANT ALL ON SCHEMA $DB_SCHEMA_MAPPING TO $DB_ADMIN_USER;
        GRANT USAGE ON SCHEMA $DB_SCHEMA_MAPPING TO $DB_APP_USER;

        REVOKE ALL ON ALL TABLES IN SCHEMA $DB_SCHEMA_MAPPING FROM PUBLIC;
        GRANT ALL ON ALL TABLES IN SCHEMA $DB_SCHEMA_MAPPING TO $DB_ADMIN_USER;
        GRANT SELECT, INSERT, UPDATE ON ALL TABLES IN SCHEMA $DB_SCHEMA_MAPPING TO $DB_APP_USER;

        ALTER DEFAULT PRIVILEGES IN SCHEMA $DB_SCHEMA_MAPPING GRANT INSERT, SELECT, UPDATE ON TABLES TO $DB_APP_USER;
        ALTER DEFAULT PRIVILEGES IN SCHEMA $DB_SCHEMA_MAPPING GRANT USAGE ON SEQUENCES TO $DB_APP_USER;
        ALTER DEFAULT PRIVILEGES IN SCHEMA $DB_SCHEMA_MAPPING GRANT EXECUTE ON FUNCTIONS TO $DB_APP_USER;
    COMMIT;
