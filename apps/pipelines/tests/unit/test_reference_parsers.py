"""Tests for reference-domain parser output."""

import json
from datetime import date

from core.ingestion import bronze_record
...
def test_parse_exchange_snapshots() -> None:
    ...
    assert json.loads(bronze_record(EXCHANGE_DATASET, sources[0])["raw_json"])["Code"] == "US"


def test_parse_instrument_snapshots() -> None:
    """Instrument parse to typed Bronze sources."""
    ...
    assert json.loads(bronze_record(INSTRUMENT_DATASET, valid[0])["raw_json"])["Code"] == "AAPL"
...
def test_parse_exchange_schedule_snapshot() -> None:
    """Exchange schedule parse to a single typed Bronze source."""
    ...
    assert json.loads(bronze_record(EXCHANGE_SCHEDULE_DATASET, source)["raw_json"])["Code"] == "XNYS"
...
def test_parse_exchange_holiday_snapshots() -> None:
    ...
    assert json.loads(bronze_record(EXCHANGE_HOLIDAY_DATASET, sources[0])["raw_json"])["Holiday"] == "New Year's Day"
