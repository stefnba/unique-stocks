...
| `control_plane/` | How is this app wired into Prefect, AWS, deployment, and runtime services? | App-specific Prefect block definitions, control registrations, automation definitions, deployment defaults, AWS resource naming, and other runtime/control-plane wiring. Generic reusable helpers still belong in `core/`.            |
...
- Keep Prefect mechanics in `core.prefect`: block registry base classes, control registry base classes, event vocabulary, automation upsert helpers, deployment sync helpers, and limit setup mechanics. `control_plane.prefect` only declares this app's blocks, controls, automations, and deployment defaults.
