...
| Exchange calendars          | dbt maps EODHD provider exchange codes to schedule endpoint codes, then applies working days, holidays, and early closes to decide expected trading days. Unknown mappings and dates outside the provider holiday horizon are surfaced instead of silently treated as open/closed. |
| Instrument lifecycle        | dbt derives provider instrument lifecycle evidence from observed EOD price bounds and terminal backfill outcomes. Latest-universe instruments without provider evidence are surfaced as lifecycle-unknown until prices or reviewed terminal coverage prove behavior.               |

The new dbt control views make the invariant observable:

| Model                                                | Purpose                                                                                                                                                                                                                         |
| ---------------------------------------------------- | ------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------- |
| `silver.int_exchange_trading_day`                    | One row per enabled EOD provider exchange and calendar date, with known/unknown calendar, holiday-calendar coverage, working-day, holiday, early-close, and trading-day flags.                                                  |
| `silver.int_eod_price_provider_instrument_lifecycle` | Provider instrument lifecycle evidence from latest universe, observed price bounds, and terminal backfill outcomes.                                                                                                             |
